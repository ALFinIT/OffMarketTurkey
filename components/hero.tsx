...
export function Hero() {
  ...
  const heroVideo =
    ...
    'https://cdn.coverr.co/videos/coverr-luxury-villa-3059/1080p.mp4'
  const fallbackImage =
    process.env.NEXT_PUBLIC_HERO_FALLBACK_IMAGE ??
    'https://images.unsplash.com/photo-1560185127-6f3705492eb6?auto=format&fit=crop&w=2400&q=80'
    ...
    <section className="relative h-[100svh] flex flex-col overflow-hidden bg-background select-none">
      ...
      <div className="absolute inset-0 anim-cut-in" style={d(100)} aria-hidden="true">
        <div className="absolute inset-0">
          <Image
            src={fallbackImage}
            alt="Luxury residence hero backdrop"
            fill
            priority
      ...
      <nav className="relative z-20 flex items-center justify-between px-4 md:px-8 lg:px-12 py-3 md:py-6 anim-cut-in" style={d(100)}>
          ...
          <div className="relative w-7 h-7 md:w-8 md:h-8 flex items-center justify-center anim-snap-in" style={d(150)}>
            ...
            <span className="relative font-sans text-sm md:text-lg text-accent-foreground leading-none">LE</span>
          </div>
        ...
        <div className="hidden lg:flex items-center gap-10">
          {["Featured", "Properties", "About", "Contact"].map((item, i) => (
            <a
              key={item}
      ...
      >
        ...
        {["Featured", "Properties", "About", "Contact"].map((item) => (
          <a
            key={item}
            href={`#${item.toLowerCase()}`}
            onClick={() => setMenuOpen(false)}
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 md:px-8 lg:px-12 md:justify-end md:pb-8">
        <div className="max-w-[1400px] mx-auto w-full text-center md:text-left">
          ...
          <div className="relative mb-2 md:mb-0">
            {["Exclusive", "Off-Market", "Properties."].map((word, i) => (
              <div key={word} className="overflow-hidden">
                <h1
                  className={`font-sans leading-[0.85] tracking-[-0.04em] uppercase anim-shutter-up ${
                    word === "Off-Market" ? "text-accent" : "text-foreground"
          ...
          {/* Copy */}
          <p className="font-mono text-[8px] sm:text-[9px] md:text-[10px] text-foreground/60 max-w-[240px] sm:max-w-xs md:max-w-md mx-auto md:mx-0 leading-relaxed anim-cut-in" style={d(800)}>
            Discover premium real estate opportunities not listed publicly. Direct access to exceptional properties.
          </p>
          ...
          <div className="flex items-center justify-center md:justify-start gap-2 md:gap-4 mt-3 md:mt-6 anim-cut-in" style={d(900)}>
            ...
            >
              <span className="relative z-10 group-hover:text-accent-foreground transition-colors duration-300">Browse Properties</span>
          ...
          <div className="mt-4 md:mt-10 flex items-end justify-center md:justify-start gap-4 sm:gap-8 md:gap-12 anim-counter-pop" style={d(950)}>
            {[
              { target: 150, suffix: "+", label: "Properties" },
              { target: 500, suffix: "+", label: "Clients" },
              { target: 95, suffix: "%", label: "Satisfaction" },
            ].map((stat, i) => (
              <div key={stat.label} className="relative">
                <span className="font-sans text-lg sm:text-xl md:text-2xl lg:text-3xl text-foreground leading-none block">
                  <Counter target={stat.target} suffix={stat.suffix} go />
                </span>
                <span className="font-mono text-xs sm:text-sm md:text-base text-foreground/40 block tracking-[0.12em] md:tracking-[0.25em] uppercase mt-0.5 md:mt-2">
                  {stat.label}
                </span>
                {i < 2 && (
      ...
      <div className="relative z-10 px-4 md:px-8 lg:px-12 pb-4 md:pb-6 hidden md:block">
        <div className="flex items-center justify-between max-w-[1400px] mx-auto w-full anim-cut-in" style={d(1000)}>
          ...
          <div className="flex items-center gap-6">
            {["LUXURY PROPERTIES", "OFF-MARKET", "EXCLUSIVE ACCESS"].map((info, i) => (
